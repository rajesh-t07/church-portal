...
const AdminSubmissions = () => {
  ...
  const fetchSubmissions = async () => {
    try {
      ...
      // Filter by user
      if (selectedUser !== 'all') {
        filtered = filtered.filter(sub => sub.userId.toString() === selectedUser);
      }
      ...
      if (yearFilter !== 'all') {
        filtered = filtered.filter(sub => {
          const submissionYear = new Date(sub.submissionDate).getFullYear().toString();
          return submissionYear === yearFilter;
        });
      }
        ...
        {allExpenses.length > 0 ? (
          <div style={{
            ...
            <div style={{
              ...
              <div style={{
                display: 'grid',
                gridTemplateColumns: '80px 120px 150px 120px 200px 100px 120px 150px 120px',
                gap: '10px',
                padding: '0 20px',
                fontWeight: 'bold',
                color: '#495057'
